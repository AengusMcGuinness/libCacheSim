...
class FifoCache {
 public:
  explicit FifoCache(uint64_t cache_size) : cache_size_(cache_size) {}

  void on_hit(obj_id_t) {}
...
obj_id_t cache_eviction_hook(void *data, const request_t *) {
...
void cache_free_hook(void *data) {
  delete static_cast<FifoCache *>(data);
}
